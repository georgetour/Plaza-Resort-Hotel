...
class AWE_Post_type extends AWE_function {
	...
	public function __construct() {
		...
		add_action( 'add_meta_boxes' ,  array( $this, 'add_meta_boxe_shop_order' ),40);
	...
	public function shop_order_column( $columns ) {
		...
		$columns['apb_type']            = __( 'Type', 'awebooking' );
		...
		switch ( $column ) {
			case 'apb_order_title':
				if ( get_post_meta($post->ID, '_shipping_last_name', true) ) {
					$username = get_post_meta($post->ID, '_shipping_last_name', true);
				} else {
					$user = get_post_meta( $post->ID, 'info_custom_order', true );
					if ( ! empty( $user['apb-name'] ) ) {
						$username = $user['apb-name'];
					} else {
						$username = __( 'Guest', 'awebooking' );
					}
			...
			case 'apb_order_arrival':
				echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( get_post_meta( $post->ID, 'from', true ) ) ) );
				break;

			case 'apb_order_departure':
				echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( get_post_meta( $post->ID, 'to', true ) ) ) );
				break;
			...
			case 'apb_order_date' :
				...
				if ( '0000-00-00 00:00:00' == $post->post_date ) {
					...
					$h_time = get_the_time( get_option( 'date_format' ), $post );
