...
class Process_ajax {
				...
				$html .= '<div class="form-item">';
					$html .= '<div class="form-elements">';
						...
						foreach ( $data_option as $item ) {
							...
							$html .= sprintf(
								'<label for="package-%1$s"><input type="checkbox" id="package-%1$s" name="package_data[id][]" class="options_operation_js package-%1$s" data-operation="%2$s" data-value="%3$s" data-nights="%6$s" data-daily="%4$s" value="%1$s"> %5$s</label>',
								absint( $item->id ),
							...
							$html .= sprintf(
								'<input data-id="%1$s" class="package_total_js package-num-%1$s" data-value="%2$s" name="package_data[total][]" type="number" min="1" value="1">',
								absint( $item->id ),
								esc_attr( $price_package )
							);
	...
	public function get_info_event() {
		...
		if ( ! empty( $info_events ) ) {
			...
			foreach ( $info_events as $info_event) {
				...
				$html .= '<p><strong>' . esc_html__( 'Room:', 'awebooking' ) . '</strong> ' . esc_html( $room->post_title ) . '</p>';
	...
	public function check_available() {
		...
		if ( $room_type_id ) {
			...
			if ( 'min-night' == $result ) {
				do_action( 'apb_unavailable_message_min-night', $room_type_id );
