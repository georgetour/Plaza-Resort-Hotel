...
add_filter( 'wp_setup_nav_menu_item', 'calluna_add_custom_nav_fields' );

// save menu custom fields
add_action( 'wp_update_nav_menu_item', 'calluna_update_custom_nav_fields', 10, 3 );

// edit menu walker
add_filter( 'wp_edit_nav_menu_walker', 'calluna_edit_walker', 10, 2 );


function calluna_add_custom_nav_fields( $menu_item ) {
 ...
 *
*/
function calluna_update_custom_nav_fields( $menu_id, $menu_item_db_id, $args ) {
		
		$check = array('icon', 'background', 'bg_position', 'megamenu', 'menutitle', 'cols_nums');
 ...
 *
*/
function calluna_edit_walker($walker,$menu_id) {

	return 'Calluna_Walker_Nav_Menu_Edit';
...
if (!class_exists('calluna_header_walker_nav_menu')) {
	class calluna_header_walker_nav_menu extends Walker_Nav_Menu {
